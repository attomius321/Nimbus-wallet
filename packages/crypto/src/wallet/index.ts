 ...
 */
export function generateWallet(initialMnemonic?: string): { mnemonic: string; address: string } {
    if (initialMnemonic && validateMnemonic(initialMnemonic, wordlist) === false) {
 ...
 */
export function derivePrivateKeyForAccount(mnemonic: string, accountIndex: number): Uint8Array {
    const seed = mnemonicToSeedSync(mnemonic, PASSPHRASE)
    const root = HDKey.fromMasterSeed(seed)
